...
func _fire_projectile() -> void:
	...
	# ✅ Trigger the same attack animation as melee
	is_attacking = true
	anim_state.travel("attack")
	anim_tree.set("parameters/attack/blend_position", last_direction)

	# ✅ Spawn projectile halfway through the animation
	await get_tree().create_timer(ATTACK_HITBOX_TIME).timeout

	var instance = arrow_projectile.instantiate()
	instance.dir = last_direction.angle()
	instance.spawnPos = global_position + Vector2(12, -7)
	instance.spawnRot = last_direction.angle()
	get_tree().get_root().add_child(instance)

	# ✅ Finish the animation with the same recovery time as melee
	await get_tree().create_timer(ATTACK_RECOVERY).timeout
	is_attacking = false
